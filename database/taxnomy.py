...
if __name__ == '__main__':
    ...
    with open("%s.genus.abundance" % name) as fqw:
        for key,value in genus_abun.items():
            fqw.write("%s\t%s\n" % (key,value))
    with open("%s.family.abundance" % name) as fqw:
        for key,value in family_abun.items():
            fqw.write("%s\t%s\n" % (key,value))
    with open("%s.order.abundance" % name) as fqw:
        for key,value in order_abun.items():
            fqw.write("%s\t%s\n" % (key,value))
    with open("%s.class.abundance" % name) as fqw:
        for key,value in class_abun.items():
            fqw.write("%s\t%s\n" % (key,value))
    with open("%s.phylum.abundance" % name) as fqw:
