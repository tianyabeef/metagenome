...
def megermismatch(tabs):
    ...
    n = len(tabs)/2
    for i in range(n):
        #XXXid  150M  mismatch Astart  mismatchsum Bstart  Blen
        mismatchall = "%s\t%s\t%s\t%s" % (tabs[i+0],int(tabs[i+0].split("\t")[2])+int(tabs[i+n].split("\t")[2]),
                                  tabs[i+n].split("\t")[3],tabs[i+n].split("\t")[1])
        ...
        if inf:
            for k,value in inf.items():
                ...
                for v in  tabs:
                    ...
                    subchot[6]=int(subchot[6].strip('M'))
                    #IDxxx  150M  amismatch stata mismatchsum startb ,blen
                    hits.append((subchot[0],subchot[1],subchot[4],subchot[3],subchot[5],subchot[6]))
                ...
                for n, M, s ,astart,bstart,blen in hits:
                    if M < max_M or s > min_s:
                        ...
                        besthits.append((n, M, s,astart,bstart,blen))
                    else:
