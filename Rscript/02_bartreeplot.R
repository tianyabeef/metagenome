...
data <- read.table(args[1], head = T, check.names = F,quote="",sep="\t",row.names=1)
pregroup <- read.table(args[2], head = F, check.names = F, row.names = 1,quote="",sep="\t")
pdf(args[3], width = 15)
group <- as.vector(pregroup[,1])
names(group) <- rownames(pregroup)
grpcol <- c("orange", "blue", "pink", "red")
names(grpcol) <- levels(pregroup[, 1])
dist = dist(t(data))
layout(t(c(1,2,2,2,3,3)))
par(mar = c(5,5,5,0))
