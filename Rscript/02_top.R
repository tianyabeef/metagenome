...
dataraw  <- read.table("@#{input}",header = T,row.names = 1,sep = "\t",check.names = F,quote = "")
group  <- read.table("@#{group}",header = F,row.names = 1,sep = "\t",check.names = F,quote = "")
...
for(g in c(1:length(g_class),0)){
  ...
  pdf(paste("@#{pdfoutput}",g,".pdf",sep=""), height = 10, width = 10)
